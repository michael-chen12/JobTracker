...
export default function BottomNav() {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 lg:hidden overflow-hidden"
        ...
        {navItems.map((item) => {
          ...
          if (item.action === 'add') {
            return (
              ...
              >
                <div className="flex items-center justify-center w-12 h-12 rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 transition-colors flex-shrink-0">
                  <Icon className="h-6 w-6" />
                </div>
                <span className="text-[10px] mt-0.5 text-gray-500 dark:text-gray-400 truncate max-w-[56px] text-center">
                  {item.label}
                </span>
              </button>
            );
