...
describe('RelationshipStrengthBadge', () => {
  ...
  test('should show tooltip with interaction count when showTooltip is true', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/2 interactions in last 30 days/)).toBeVisible();
    });
  });
  ...
  test('should handle singular interaction count correctly', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/1 interaction in last 30 days/)).toBeVisible(); // "interaction" not "interactions"
    });
  });
});
...
describe('InteractionsList', () => {
  ...
  test('should render correct interaction count', () => {
    ...
    const items = screen.getAllByRole('article'); // Assuming InteractionItem uses article
  ...
  test('should pass correct isDeleting prop to InteractionItem', () => {
    ...
    // First interaction should have delete button disabled
    // (This would need to be verified in InteractionItem tests)
  });
});
